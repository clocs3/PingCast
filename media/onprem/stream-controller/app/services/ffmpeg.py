...
_HLS_ROUTE_PREFIX = "hls:route:"
_RECONCILE_LOCK_KEY = "lock:hls-route-reconcile"
...
def _node_internal_ip(node_name: str) -> Optional[str]:
...
def resolve_hls_route_target(
    ...
    last_reason = "ffmpeg job pod not scheduled yet"
    while time.time() < deadline:
        job_pods = core_api.list_namespaced_pod(
            namespace=namespace,
            label_selector=f"job-name={job_name}",
        ).items

        ffmpeg_pod = next((pod for pod in job_pods if pod.metadata and pod.metadata.deletion_timestamp is None), None)
        node_name = ffmpeg_pod.spec.node_name if ffmpeg_pod and ffmpeg_pod.spec else None

        if node_name:
            hls_pods = core_api.list_namespaced_pod(
                namespace=namespace,
                label_selector="app=nginx-hls",
            ...
            hls_pod = next(
                (
                    ...
                    and _is_pod_ready(pod)
                    and pod.metadata
                    and pod.metadata.deletion_timestamp is None
                ),
                None,
            )
            if hls_pod and hls_pod.status:
                if settings.HLS_USE_NODE_INTERNAL_IP:
                    node_ip = _node_internal_ip(node_name)
                    if node_ip:
                        return f"{node_ip}:{settings.HLS_NODE_ENDPOINT_PORT}"

                if hls_pod.status.pod_ip:
                    return f"{hls_pod.status.pod_ip}:80"

            last_reason = f"nginx-hls pod not ready on node {node_name}"
        else:
            last_reason = "ffmpeg job pod not scheduled yet"

        time.sleep(poll_interval_seconds)

    raise TimeoutError(f"failed to resolve hls route target for {job_name}: {last_reason}")


def _decode_redis_mapping(data):
    if not data:
        return {}
...
def reconcile_live_hls_routes() -> None:
    if not settings.ENABLE_HLS_ROUTE_DISCOVERY:
        return

    redis_client = get_redis()
    lock_acquired = bool(redis_client.set(_RECONCILE_LOCK_KEY, "1", nx=True, ex=settings.HLS_ROUTE_RECONCILE_LOCK_SECONDS))
    if not lock_acquired:
        return

    try:
        for live_key in redis_client.scan_iter(f"{_LIVE_KEY_PREFIX}*"):
            live_data = _decode_redis_mapping(redis_client.hgetall(live_key) or {})
            if live_data.get("status") != "running":
                continue

            container_name = str(live_data.get("container_name", "")).strip()
            stream_key = live_key.split(":")[-1].strip()
            if not container_name or not stream_key:
                continue

            try:
                target = resolve_hls_route_target(container_name, timeout_seconds=2.0, poll_interval_seconds=0.2)
            except Exception as exc:
                print(
                    f"[HLS ROUTE RECONCILE ERROR] stream_key={_mask_stream_key(stream_key)}, "
                    f"job={container_name}, error={repr(exc)}"
                )
                continue

            route_key = f"{_HLS_ROUTE_PREFIX}{stream_key}"
            current_target = redis_client.get(route_key)
            ttl = redis_client.ttl(live_key)
            ttl_seconds = ttl if isinstance(ttl, int) and ttl > 0 else 60 * 10
            if current_target != target:
                redis_client.set(route_key, target, ex=ttl_seconds)
                redis_client.hset(
                    live_key,
                    mapping={
                        "hls_route_target": target,
                        "hls_route_status": "ready",
                        "hls_route_error": "",
                    },
                )
            else:
                redis_client.expire(route_key, ttl_seconds)
    finally:
        try:
            redis_client.delete(_RECONCILE_LOCK_KEY)
        except Exception:
            pass
